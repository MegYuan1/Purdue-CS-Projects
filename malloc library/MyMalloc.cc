...
Allocator::allocateObject( size_t size )
{
    ...
    if(DEBUG)
    	printf("orig_size:%d round_size:%d real_size:%d\n",size, round_size, real_size);
  	
  	//Check existence of free blocks inside head_arr[list_index]
  	/*ent=(ObjectHeader *)findFreeBlock(real_size,list_index);
  	if(ent!=NULL){
  		ent->_flags=ObjAllocated;
  		foot=(ObjectHeader *)((char *)ent+ent->_objectSize-sizeof(ObjectHeader));
  		foot->_flags=ObjAllocated;
  		return (void *)((char *)ent+2*sizeof(ObjectHeader));
  	} */	
  	...
  	if(mallocPolicy){
  		for(i=list_index;i<=64;i++){
  				...
  				if(ent1!=NULL){
  					...
  					//printf("Split... Orig:%d Part1:%d Part2:%d\n",ent1->_objectSize,real_size,size2);
	...
	lastLoc=(void *)((char *)mem+real_size + 2*sizeof(ObjectHeader));
  	...
  	//o=(ObjectHeader *)((char *)mem-2*sizeof(ObjectHeader));
  	//printf("\no->flags=%d o->size=%d\n",o->_flags,o->_objectSize);  	
